#[cfg(test)]
mod adc_test {
    use crate::cpu::cpu::CPU;
    use crate::helpers::bitwise::split_word_into_bytes;

    #[test]
    fn adc_test_immediate() {
        let mut cpu = CPU::new();

        let bytes = [
            0x69, 0x7F
        ];

        cpu.registers.acc = 0x80;
        cpu.flags.carry = true;
        cpu.execute_instruction_sequence(&mut bytes.as_slice());

        assert_eq!(cpu.registers.acc, 0x00);
        assert_eq!(cpu.flags.carry, true);
        assert_eq!(cpu.flags.zero, true);
        assert_eq!(cpu.flags.negative, false);
        assert_eq!(cpu.flags.overflow, false);
    }

    #[test]
    fn adc_test_immediate_negative() {
        let mut cpu = CPU::new();

        let bytes = [
            0x69, 0x50
        ];

        cpu.registers.acc = 0x50;
        cpu.flags.carry = false;
        cpu.execute_instruction_sequence(&mut bytes.as_slice());

        assert_eq!(cpu.registers.acc, 0xA0);
        assert_eq!(cpu.flags.carry, false);
        assert_eq!(cpu.flags.zero, false);
        assert_eq!(cpu.flags.negative, true);
        assert_eq!(cpu.flags.overflow, true);
    }

    #[test]
    fn adc_test_absolute() {
        let mut cpu = CPU::new();
        let (high_byte, low_byte) = split_word_into_bytes(0x2233);
        cpu.memory[0x2233] = 0x20;
        let bytes = [
            0x6D, low_byte, high_byte
        ];

        cpu.execute_instruction_sequence(&mut bytes.as_slice());

        assert_eq!(cpu.registers.acc, 0x20);
    }

    #[test]
    fn adc_test_x_indexed_absolute() {
        let mut cpu = CPU::new();
        let (high_byte, low_byte) = split_word_into_bytes(0x2230);
        cpu.memory[0x2233] = 0x20;
        let bytes = [
            0x7D, low_byte, high_byte
        ];

        //X + ACC
        cpu.registers.xr = 3;
        cpu.execute_instruction_sequence(&mut bytes.as_slice());

        assert_eq!(cpu.registers.acc, 0x20);
    }

    #[test]
    fn adc_test_y_indexed_absolute() {
        let mut cpu = CPU::new();
        let (high_byte, low_byte) = split_word_into_bytes(0x2230);
        cpu.memory[0x2233] = 0x20;
        let bytes = [
            0x79, low_byte, high_byte
        ];

        //Y + ACC
        cpu.registers.yr = 3;
        cpu.execute_instruction_sequence(&mut bytes.as_slice());

        assert_eq!(cpu.registers.acc, 0x20);
    }

    #[test]
    fn adc_test_zero_page() {
        let mut cpu = CPU::new();
        cpu.memory[0x33] = 0x20;
        let bytes = [0x65, 0x33];

        cpu.execute_instruction_sequence(&mut bytes.as_slice());

        assert_eq!(cpu.registers.acc, 0x20);
    }

    #[test]
    fn adc_test_x_indexed_zero_page() {
        let mut cpu = CPU::new();
        cpu.memory[0x33] = 0x20;
        let bytes = [0x75, 0x30];

        //X + Zero Page (0x30 + 3 = 0x33)
        cpu.registers.xr = 3;
        cpu.execute_instruction_sequence(&mut bytes.as_slice());

        assert_eq!(cpu.registers.acc, 0x20);
    }

    #[test]
    fn adc_test_x_indexed_zero_page_indirect() {
        let mut cpu = CPU::new();
        cpu.memory[0x4030] = 0x10;
        //0x20 (0x30) + 0x21 (0x40) = 0x4030
        cpu.memory[0x20] = 0x30;
        cpu.memory[0x21] = 0x40;

        let bytes = [0x61, 0x1D];

        cpu.registers.xr = 3;
        cpu.registers.acc = 0x10;
        cpu.execute_instruction_sequence(&mut bytes.as_slice());

        assert_eq!(cpu.registers.acc, 0x20);
    }

    #[test]
    fn adc_test_zero_page_indirect_y_indexed() {
        let mut cpu = CPU::new();
        cpu.memory[0x4032] = 0x10;
        //0x20 (0x30) + 0x21 (0x40) = 0x4030
        cpu.memory[0x22] = 0x30;
        cpu.memory[0x23] = 0x40;

        let bytes = [0x71, 0x22];

        cpu.registers.yr = 2;
        cpu.registers.acc = 0x10;
        cpu.execute_instruction_sequence(&mut bytes.as_slice());

        assert_eq!(cpu.registers.acc, 0x20);
    }
}